...
| **D21** | **Venda e mensalidade não geram `payments`.** Só a conclusão de atendimento escreve lá, e `gatewayFeesTotal` soma `payments` | É a causa localizada de D7: R$ 145 no crédito e R$ 149 de mensalidade no crédito não debitam taxa nenhuma, com o `paymentMethod` gravado nos dois fatos | médio | ledger · DRE |
