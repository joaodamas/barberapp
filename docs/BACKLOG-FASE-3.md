...
| ~~**D20**~~ | ~~A fatura de mensalidade não é lida por visão financeira nenhuma~~ | ✅ **fechado na Rodada 3.2.** A receita de mensalista sai da fatura **paga**, não de `status === "ativo"`. Entra em `bruta` com nome próprio (`mensalidades`); o MRR contratado continua exposto à parte. **Verificado na tela**: R$ 99,00 na árvore do DRE | — | ledger · DRE · financeiro |
| **D20-orig** | **A fatura de mensalidade não é lida por nenhuma visão financeira.** `useFinanceiro` não passa `subscription_invoices` a lugar nenhum | G2 criou o lastro e ele está órfão: a receita de mensalista continua saindo de `status === "ativo"`. A decisão da Rodada 3 é qual dos dois números o DRE usa | decisão de modelo | ledger · DRE · financeiro |
...
| ~~**D29**~~ | ~~O pagamento de SERVIÇO não dizia de onde veio~~ | ✅ **fechado na Rodada 3.1.** G1.6 declarou `PaymentDoc.origin` e deu o campo a produto e mensalidade; o gatilho de conclusão de atendimento — que já existia — passou despercebido. **A maior fonte de receita nascia sem origem**, e a Rodada 3.2 precisa separar receita por origem para o DRE. Achado ao LER o documento gravado durante a verificação do estorno, não por teste. Corrigido + teste estrutural que varre quem escreve em `payments`. **Verificado na tela**: dois pagamentos de serviço lado a lado, o de antes com `origin: undefined` e o de depois com `origin: "servico"` | — | ledger · DRE |
| ~~**D22**~~ | ~~O estorno não existe~~ | ✅ **fechado na Rodada 3.1.** `registrarEstorno` nas três origens, com id derivado do fato + chave, motivo obrigatório e as duas datas (competência × caixa). Taxa NÃO recobrada: a perda emerge da soma do par. **Verificado na tela** nas três origens | — | ledger · DRE · Fluxo |
| **D22-orig** | ~~O estorno não existia.~~ `refunds` tem regra, tem caminho e ninguém escreve. `cancelBooking` calcula a devolução e grava em `bookings.refundedAmount`, campo que **nenhuma linha do produto lê** | O dinheiro devolvido não existe no DRE, no fluxo nem em `payments`. Com pagamento em três origens, a assimetria virou tripla | decisão de modelo | ledger · DRE · Fluxo |
| ~~**D23**~~ | ~~Venda e mensalidade não têm caminho de desfazer~~ | ✅ **fechado na Rodada 3.1.** "Devolver" na Loja (por quantidade, com valor derivado), no atendimento concluído e na fatura paga. Venda devolve estoque via `kind: "ajuste"` com `refundOf`, e reverte a comissão somando linha negativa. **Verificado na tela**: 3 un. vendidas, 1 + 2 devolvidas, soma das comissões = 0 | — | Loja · Mensal · ledger |
| **D23-orig** | ~~Venda e mensalidade não tinham caminho de desfazer.~~ Serviço tem reversão; produto e mensalidade, não | Venda registrada por engano é irreversível pela interface, com estoque baixado, pagamento gravado e taxa cobrada. O único caminho é editar o banco à mão | decisão de modelo | Loja · Mensal · ledger |
...
| **D30** | **Falha de infraestrutura expulsa o dono do produto sem contexto.** Com o Firestore indisponível, o tenant não resolve e a aplicação redireciona para a landing | A camada de tenant transforma *"não consegui carregar"* em *"você não pertence a este painel"*. É separado do D27, que é sobre coleção: aqui o dono perde o painel inteiro sem uma frase que explique. Descoberto ao tentar exercer o estado de erro em tela | médio | tenant · todas as telas |
| ~~**D31**~~ | ~~`caixaDoDia`: os filhos não somam o cabeçalho~~ | ✅ **fechado.** `total` contava todas as recebidas e as três parcelas filtravam por método: atendimento concluído sem meio informado entrava no cabeçalho e em parcela nenhuma. Mesma forma do defeito do CMV — e a função irmã `caixaDiario` já tinha sido corrigida na 3.2, esta ficou para trás. Ganhou `naoInformado` e teste de invariante. Achado pela auditoria de DENSIDADE, que foi procurar duplicação | — | Hoje · ledger |
| **D32** | **"Taxas por método" mostra tabela de preço fixa de Stone e InfinitePay** | `tenant.ts` declara que o produto se RECUSA a inventar taxa de mercado, e a tela exibe uma. Mesma forma do exemplo proibido em UI-UX-GUIDELINES §13. Precisa de decisão: remover, ou ler das taxas configuradas da barbearia | baixo | Financeiro |
| **D33** | **Dois atalhos anunciam número que o destino não tem.** "Fluxo de Caixa" mostra só entradas enquanto o destino encabeça `fluxo.saldo`; "Despesas" soma tudo sem recorte de período | O dono clica esperando um número e encontra outro — a mesma classe de "Quanto sobrou" × "DRE Gerencial", agora em valor em vez de nome | baixo | Financeiro |
| **D34** | **"Crescimento líquido de mensalistas" conta todos os ativos como novos** | `SubscriberDoc` não guarda `createdAt` — o número **não é calculável**. UX-05 deixou a palavra "novos" de propósito: um rótulo verdadeiro deixaria a legenda honesta embaixo de um KPI que continua chamando `ativos − cancelados` de crescimento. É dado faltando, não linguagem. (Reabre o P1-10) | médio | Financeiro |
| **D35** | **O MRR de Mensalistas sai de `subscribers.price × ativo`** | Fonte que `MAPA-DE-FONTES.md` §3 marca como *"nunca pode entrar"*, exibida ao lado do bloco que já sai das faturas. Duas fontes para a mesma grandeza, na mesma tela | médio | Mensalistas · ledger |
| **D36** | **Não existe tela que crie plano de mensalista.** `plans` só é escrita pelo script de semeadura | E o `RecursoBloqueado` de Mensalistas **vende** "cadastra planos de assinatura". Dois textos mandavam o dono criar plano na Loja e em Serviços; nenhuma das duas faz isso. UX-05 tirou as afirmações falsas — falta a TELA, não o texto | médio | Mensalistas |
| **D37** | **Cinco telas mostram R$ 0,00 durante carregamento e erro**, ao lado do próprio aviso de erro | É o D27 com outra roupa: o número zerado afirma "não houve movimento" enquanto o aviso diz "não consegui ler". Estrutura, não texto | baixo | UI/UX · todas |
| ~~**D25**~~ | ~~`cash_entries` e `client_occurrences` são coleções mortas~~ | ✅ **`cash_entries` FECHADO na Rodada 3.1.** `registrarMovimentoDeCaixa` com cinco tipos — sangria, troco, aporte, pagamento de comissão, ajuste. Exclusividade **por construção**: enum fechado que não contém venda/atendimento/mensalidade/compra/despesa. Sinal derivado do tipo, motivo obrigatório, idempotência por chave. Escrita direta FECHADA nas regras (era `write: if isOwnerOf`). **Verificado na tela**: quatro lançamentos, saldo R$ 565 somando os sinais; e o dono logado tomou **403 ao criar e ao editar** direto. `client_occurrences` continua gap de produto no Bloco 2 | — | Fluxo · ledger |
| **D25-orig** | ~~`cash_entries` e `client_occurrences` eram tidas como mortas.~~ | ✅ **investigado: NÃO são resíduo.** PRD §13 descreve as duas. `client_occurrences` aciona a exigência de pagamento antecipado (gap de produto); `cash_entries` é o livro caixa, e existe para movimentos SEM outro fato — sangria, troco, aporte, pagamento de comissão. Vira decisão da Rodada 3, não remoção | — | — |
...
| ~~**D3**~~ | ~~CMV soma as compras do período~~ | ✅ **fechado na Rodada 3.2.** Sai do `unitCost` congelado em cada venda, e a devolução o reduz. Compra deixa de ser custo do período e vira saída de caixa (3.3). **Reconciliação**: o sistema debitava 180, o ledger dizia 116 — agora os dois dizem 116 | — | ledger · DRE · 6 visões |
| **D3-orig** | **CMV soma as compras do período**, não o custo do que foi vendido | Num mês de reposição, o lucro da loja despenca sem nada ter piorado. Cresce com o uso | alto | ledger · DRE · 6 visões |
...
| ~~**D7**~~ | ~~Venda de produto não gera taxa de maquininha~~ | ✅ **fechado na Rodada 3.2.** G1.6 criou o pagamento; a leitura passou a somá-lo. **Reconciliação**: taxas de 4,00 → 7,85, exatamente o ledger (4,14 de serviço + 3,71 de produto) | — | ledger · DRE |
| **D7-orig** | Venda de produto **não gera taxa de maquininha** | A taxa de cartão sobre produto some do custo | médio | ledger · DRE |
...
| ~~**D1 / D5**~~ | ~~Taxas e imposto arredondam ao real~~ | ✅ **fechado na Rodada 3.2.** Os dois ao centavo. O erro era pequeno, sistemático e sumia exatamente onde seria conferido: o extrato da maquininha e a guia do Simples. Imposto 41,00 → 40,80; taxas 8,00 → 7,85 | — | ledger · DRE |
| **D1 / D5-orig** | Taxas e imposto arredondam **ao real**, não ao centavo | Sistemático e sempre a favor do lucro aparente | baixo | ledger · DRE |
