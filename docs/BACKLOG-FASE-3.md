...
| **D25** | **`cash_entries` e `client_occurrences` são coleções mortas.** Declaradas em `paths.ts` e nas regras, zero escritas e zero leituras | Mesma classe de `inventory_movements` antes de G1: coleção que parece implementada. `cash_entries` não tem propósito escrito em documento nenhum | baixo | — |
