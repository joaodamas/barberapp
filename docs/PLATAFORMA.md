# JPBarber — documentação técnica e funcional
...
(`osiqueira.jpbarber.com.br`), a própria marca, o próprio catálogo e o próprio
...
`osiqueira.jpbarber.com.br` → `slugFromHost` extrai `osiqueira` →
...
JPBarber e a nota de qualidade é compartilhada.
