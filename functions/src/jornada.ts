 ...
 *
 * ⚠️ **PAR OBRIGATÓRIO com `web/src/lib/jornada.ts`.** A mesma decisão roda nos
 * dois lados: aqui para decidir o que o cliente pode reservar, lá para pintar a
 * tela do dono e contar capacidade. Divergir significa a tela oferecer um
 * horário que o servidor recusa — e o cliente lendo "não foi possível" sem
 * motivo visível. Os dois arquivos mudam juntos, sempre.
 *
 * Este lado é a AUTORIDADE: a tela pode estar desatualizada no navegador de
 * alguém, o servidor não. `availability`, `createBooking` e `rescheduleBooking`
 * chamam daqui — as três resoluções de jornada que existiam separadas, cada uma
